...
class HighlightingSettings(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ext_frame = ttk.LabelFrame(input_frame, text="Action List")
        ...
        self.color_button = ttk.Button(a, text="Add", command=self.add)
        self.color_button.grid(row=0, column=4)

        self.tlabel = ttk.Label(a, text="", anchor="w", width=2)
        self.tlabel.grid(row=0, column=2)

        self.text_contains_label = ttk.Label(a, text="Text contains", anchor="w")
        ...
        self.color_table_frame = ttk.LabelFrame(e, text="Visual")
        self.color_table_frame.pack(fill="both", expand=True, padx=0, pady=0)

        self.color_table = ttk.Treeview(self.color_table_frame, columns=("Facility", "Severity", "Text contains", "Action"), show="headings")
        ...
        self.color_table.column("Text contains", width=300, minwidth=50, stretch=tk.NO)
        self.color_table.heading("Action", text="Action")
        self.color_table.column("Action", width=150, minwidth=50, stretch=tk.NO)
        ...
        new_frame = ttk.LabelFrame(input_frame, text="Action")
        ...
        self.allow_alert_check = ttk.Checkbutton(new_frame, text="Show alarms window", variable=self.allow_alert_var)
        ...
        self.allow_sound_check = ttk.Checkbutton(frame0, text="Play sound file", variable=self.allow_sound_var)
        self.allow_sound_check.grid(row=0,column=0, padx=3)

        sound_file_button = ttk.Button(frame0, text="Listen", width=6, command=self.listen_sound_file)
        sound_file_button.grid(row=0,column=1)

        # Поле выбора звукового файла
        self.sound_file_var = tk.StringVar()
        self.sound_file_entry = ttk.Entry(frame0, textvariable=self.sound_file_var, width=10)
        self.sound_file_entry.grid(row=1, column=0, padx=(20,5))
        sound_file_button = ttk.Button(frame0, text="Browse", width=0, command=self.browse_sound_file)
        ...
        self.sound_repeat_label = ttk.Label(frame0, text="Play count")
        ...
        self.allow_external_check = ttk.Checkbutton(frame0, text="Run program", variable=self.allow_external_var)
        self.allow_external_check.grid(row=3, column=0, padx=0)

        self.program_var = tk.StringVar()
        self.program_entry = ttk.Entry(frame0, textvariable=self.program_var, width=10)
        self.program_entry.grid(row=4, column=0, padx=(20, 5))
        program_button = ttk.Button(frame0, text="Browse", width=0, command=self.browse_program)
...
#        self.update_color_table()
    ...
    def update_color_table(self):
        ...
        for facility_severity, action_str in self.color_mapping.items():
            ...
            if "Show alarm" in action_str:
                actions.append("Show alarm")
            if "Play sound" in action_str:
                actions.append("Play sound")
            if "Run program" in action_str:
                actions.append("Run program")
            action_display= " AND ".join(actions)
    ...
    def add(self):
        ...
        if self.allow_sound_var.get():
            sound_file = self.sound_file_var.get()
            if sound_file:
                flags.append("Play sound")
        if self.allow_external_var.get():
            program = self.program_var.get()
            if program:
                flags.append("Run program")
        # Сформируем строку для столбца действия
        action_str = " AND ".join(flags) if flags else ""
