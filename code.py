...
class SyslogServer(tk.Tk):
    def __init__(self):
        ...
        self.start_server_button = ttk.Button(self.toolbar_frame, text="Start", command=self.start_server)
        self.start_server_button.pack(side="left", padx=5, pady=5)

        self.setup_button = ttk.Button(self.toolbar_frame, text="Setup", command=self.setup_vss)
        self.setup_button.pack(side="left", padx=5, pady=5)

        self.processing_button = ttk.Button(self.toolbar_frame, text="Processing", command=self.processing_vss)
        self.processing_button.pack(side="left", padx=5, pady=5)

        self.highlighting_button = ttk.Button(self.toolbar_frame, text="Highlighting", command=self.highlighting_vss)
        self.highlighting_button.pack(side="left", padx=5, pady=5)

        self.clear_logs_button = ttk.Button(self.toolbar_frame, text="Clear Logs", command=self.clear_logs)
        self.clear_logs_button.pack(side="left", padx=5, pady=5)

        self.terminate_button = ttk.Button(self.toolbar_frame, text="Terminate", command=self.close)
        ...
        self.filter_button = ttk.Button(self.display_frame, text="Filter", command=self.open_filter_windows)
        ...
        self.toggle_display_real_time_button = ttk.Checkbutton(self.display_mode_frame, text="Display in Real Time",
        ...
        self.toggle_display_file_button = ttk.Checkbutton(self.display_mode_frame, text="Display from File",
                                                          command=self.toggle_display_mode)
        self.toggle_display_file_button.pack(side="left", padx=0, pady=0)

        self.view_file_label = ttk.Label(self.display_frame, text="File:")
        self.view_file_label.pack(side="left", padx=10, pady=15)

        self.view_file_options = ["None"]
        self.load_files()
        ...
        self.display_message_frame = ttk.LabelFrame(self, text="Display Message")
        ...
        self.log_tree = ttk.Treeview(self.display_message_frame, columns=(
            "Priority", "Timestamp", "Version", "Hostname", "Appname", "Process_ID", "Message", "Structured Data",
            "Mesage_ID"
        ), show="headings")
        ...
        self.log_tree.column("Priority", minwidth=50, width=80, stretch=tk.NO)
        self.log_tree.column("Timestamp", minwidth=50, width=350, stretch=tk.NO)
        self.log_tree.column("Version", minwidth=50, width=80, stretch=tk.NO)
        self.log_tree.column("Hostname", minwidth=50, width=120, stretch=tk.NO)
        self.log_tree.column("Appname", minwidth=50, width=120, stretch=tk.NO)
        self.log_tree.column("Process_ID", minwidth=50, width=100, stretch=tk.NO)
        self.log_tree.column("Message", minwidth=50, width=750, stretch=tk.NO)
        self.log_tree.column("Structured Data", minwidth=50, width=200, stretch=tk.NO)
        self.log_tree.column("Mesage_ID", minwidth=50, width=100, stretch=tk.NO)
    ...
    def sort_priority(self, reverse=False):
        data = [(int(self.log_tree.set(child, "Priority")), child) for child in self.log_tree.get_children('')]
        data.sort(reverse=reverse)
        for index, (val, child) in enumerate(data):
            self.log_tree.move(child, '', index)

    def sort_version(self, reverse=False):
        data = [(int(self.log_tree.set(child, "Version")), child) for child in self.log_tree.get_children('')]
        data.sort(reverse=reverse)
        for index, (val, child) in enumerate(data):
            self.log_tree.move(child, '', index)

    def sort_process_id(self, reverse=False):
        data = [(self.log_tree.set(child, "Process_ID"), child) for child in self.log_tree.get_children('')]
        if not reverse:
    ...
    def sort_timestamp(self, reverse=False):
        data = [(self.log_tree.set(child, "Timestamp"), child) for child in self.log_tree.get_children('')]
        if not reverse:
    ...
    def start_server(self):
        if self.display_real_time and self.view_file_combobox.get() == "None":
            self.run_server()
        ...
        elif self.display_real_time and self.view_file_combobox.get() != "None":
        ...
        elif not self.display_real_time and self.view_file_combobox.get() != "None":
        ...
        try:
            with open("server_settings.json", "r") as f:
                ...
                if self.tcp_enabled:
                    ...
                    protocol_text += "TCP: Server disabled       "
                if self.udp_enabled:
                    protocol_text += f"UDP: {self.udp_address}:{self.udp_port}      "
                else:
                    protocol_text += "UDP: Server disabled       "
